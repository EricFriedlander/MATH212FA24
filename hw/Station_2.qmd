...
> **TASK:** Edit the bullet list below to add a short description in your own words describing what each function does.
...
> **TASK--create a barchart for two categorical variables. Try changing `"dodged"` to `"stacked"` and `"fill"`. Choose one and briefly explain why you chose that barchart:**

```{r}
#gf_bar( ~ CatVariable1, fill = ~ CatVariable2, data = state_data, position = "dodged")
```
