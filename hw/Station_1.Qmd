...
4.  The Console/Qmd file and the Quarto output document are *separate entities*. Suppose you've run something, saved something, opened a data set, or opened loaded a package in the Console. It exists *only* in the Console, not in the Quarto document. Think of the Console as a "practice area" -- a place where you can play around and try stuff. Once you've settled on a piece of code and you want it to appear in the "official" document (that you'll turn in to me), put it into the Quarto document (within a "chunk", of course). Note that when you click the green run button, it runs the code in the console not the Quarto document. This can occasionally cause inconsistencies if you run your code chunks out of order while you're writing your document. When you ultimately press `Render`, Quarto will re-run the WHOLE document in order from the beginning to generate the output file.

## E. R packages

In addition to its core features, R provides many more features through a (large) number of packages. To use a package, it must be installed (one time), and loaded (each session). You install the package by clicking the "Tools \> Install Packages..." in the toolbar, then typing the name of the package. To load the package in the Console, you can type `library(package_name)`, or just click the button next to the package in the "Packages" tab. However, if you are going to use a package within an Quarto document, you must "call" the package within your particular Quarto document. (Why? Because the Console and the Quarto document are separate entities!)

For example: We will be using the `tidyverse` package a lot this term. <!-- The package is already installed on Wooster's RStudio server.  --> (If you're using a desktop version of RStudio you will need to install it if you haven't used it before.) Load the package in the Console by typing `library(tidyverse)` in the Console. Now we still need to load it *in this Quarto document*.
...
-   Download the `USStates_fall2021.csv` file from Canvas. It lives under the `Data sets` tab.
