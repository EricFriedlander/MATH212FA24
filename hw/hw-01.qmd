...
This HW will go through much of the same workflow we've demonstrated in class. The main goal is to reinforce our demo of R and RStudio, which we will be using throughout the course both to learn the statistical concepts discussed in the course and to analyze real data and come to informed conclusions.
...
-   Make a subfolder in your `hw` director to store this homework.

-   Log into Canvas, navigate to HW-01 and upload the `hw-01.qmd` and `parks.csv` file into the folder your just made. 

## R and R Studio

[Here](https://mat212fa24.netlify.app/slides/02-big-picture.html#/rstudio-ide) are the components of the RStudio IDE and [here](https://mat212fa24.netlify.app/slides/02-big-picture.html#/quarto-1) are the components of an Quarto (.qmd) file.
...
This he will focus on the following variables:
...
-   Fit and display a linear regression model of the relationship between `mean_spend` and `mean_pts_near`. Use the `lm` function and display the model in a tidy format.
...
[^1]: The "Workflow & formatting" grade is to assess the reproducible workflow and document format. This includes having a neatly organized document with readable code and your name and the date in the YAML.
