...
2.  Suppose we fit the regression model from the previous question. Which genre would you expect to have the smallest residuals, on average (*residual = observed revenue - predicted revenue*)?
    ...
    Post your response on Poll Everywhere.
...
3.  \[Time permitting\] Discuss the following: Notice in the graph above that `budget` and `gross` are log-transformed. Why are the log-transformed values of the variables displayed rather than the original values (in U.S. dollars)? **Post your group's response in the AE 01 Movie Budgets comments on Poll Everywhere.**
