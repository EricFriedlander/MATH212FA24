...
::: appex
📋 [AE 18 - Intro to Logistic Regression](https://mat212fa24.netlify.app/ae/ae-17-multicollinearity)

- Open up AE 18 and Complete Exercise 0
:::
...
## What are the odds of developing heard disease?
...
heart_disease_fit <- glm(high_risk ~ age, data = heart_disease, family = "binomial")
...
```{r}
augment(heart_disease_fit) |> select(.fitted, .resid)
```
...
```{r}
predict(heart_disease_fit, new_data = heart_disease, response = "response")
```
...
```{r}
predict(heart_disease_fit, new_data = heart_disease, type = "terms")
```
